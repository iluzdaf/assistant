...
- Add one unchecked checkbox item per PR labeled `needs-review`.
...
- Use `needs review` wording only for PRs that specifically require human PR review.
...
- Leave completed PR checkbox lines checked; do not re-add them unless the source PR has new relevant activity or returns to `needs-review`.
