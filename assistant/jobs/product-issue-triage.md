...
- If an open issue has any label in `Human-gate issue labels`, add or update it in the Bear review card and do not move it past that gate.
...
- If the issue can be handled by an agent and has no human-gate label, process it directly according to the product repository workflow doc.
...
- After successful agent processing, update only non-gate labels according to the product repository workflow doc.
...
- Open issues carrying any `Human-gate issue labels` were added or updated in the Bear review card and were not moved past the gate by an agent.
- No agent removed a human-gate label, replaced a human-gate label, or added a forward-progress label that moves an issue past a human gate.
