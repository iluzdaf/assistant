...
- If no linked issue is found, add `blocked` to the PR with the missing-link evidence and stop that PR.
...
- If unresolved open questions, planning decisions, or scope changes remain, add `blocked` to the PR with the exact blocker and stop that PR.
...
- PRs with missing links, unresolved questions, ambiguous planning decisions, or scope changes were marked `blocked` and not moved to `needs-plan-approval`.
...
- A run-log entry records this job's status, planned PRs, skipped PRs, blocked PRs, outputs, and any evidence gaps after the checks above pass.

## Outputs

- Updated draft PRs with plans and `needs-plan-approval`.
- Blocked PRs with exact blocker evidence when planning cannot proceed.
