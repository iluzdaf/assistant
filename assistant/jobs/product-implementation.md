...
- Purpose: scan configured product repositories for PRs labeled `plan-approved`, claim each approved plan by replacing `plan-approved` with `in-progress`, lock the claimed PR conversation, dispatch exactly one separate implementation agent for each claimed PR, and stop.
...
- Give the delegated implementation agent the PR URL, repository, head branch, linked issue, assistant workflow, product supplement path, latest gathered context, and this required behavior:
  ...
  - Do not check for or respond to new PR comments while the PR is locked and labeled `in-progress`.
  - If implementation findings introduce new scope, unresolved product decisions, credentials, external dependencies, or ambiguity, write the exact stop reason in the PR, leave labels unchanged, unlock the PR conversation, and stop.
  ...
  - Replace `in-progress` with `needs-review` and unlock the PR conversation only after implementation and agent-verifiable smoke tests are complete.
...
- Each dispatched PR had `plan-approved` replaced with `in-progress` and the PR conversation locked before dispatch.
...
- Any locking failure after claiming kept `in-progress` and was recorded with exact evidence.
- Any dispatch failure after claiming and locking kept `in-progress`, kept the PR conversation locked, and was recorded with exact evidence.
...
- PRs moved from `plan-approved` to `in-progress` and locked after successful claiming.
